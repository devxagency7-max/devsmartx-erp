...
import { revenueCategoryStepSchema, type RevenueCategoryStepInput } from '../../validation/revenue.schema';
...
export function StepCategory({ data, isFirstStep, onNext, onBack, onCancel }: StepCategoryProps) {
  ...
  const revenueCategories = categories.filter(
    (c) => c.applicableTypes.includes(TransactionType.Revenue) && c.status === 'active',
  );

  const { handleSubmit, setValue, watch, formState: { errors } } = useForm<RevenueCategoryStepInput>({
    resolver: zodResolver(revenueCategoryStepSchema),
    defaultValues: { categoryId: data.categoryId, categoryName: data.categoryName },
  });

  const selectedId = watch('categoryId');

  function onSubmit(values: RevenueCategoryStepInput) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-1.5">
        <Label>{t('revenue.wizard.selectCategory')}</Label>
        {isLoading ? (
        ...
        {errors.categoryId && (
          <p className="text-xs text-[hsl(var(--destructive))]">
            {t(errors.categoryId.message ?? 'revenue.validation.categoryRequired')}
          </p>
        )}
      </div>
