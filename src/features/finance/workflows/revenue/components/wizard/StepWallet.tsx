...
import { revenuePaymentSourceStepSchema, type RevenuePaymentSourceStepInput } from '../../validation/revenue.schema';
...
export function StepWallet({ data, isFirstStep, onNext, onBack, onCancel }: StepWalletProps) {
  ...
  const { handleSubmit, setValue, watch, formState: { errors } } = useForm<RevenuePaymentSourceStepInput>({
    resolver: zodResolver(revenuePaymentSourceStepSchema),
    defaultValues: { paymentSourceId: data.paymentSourceId, paymentSourceName: data.paymentSourceName, currency: data.currency },
  });

  const selectedId = watch('paymentSourceId');

  function onSubmit(values: RevenuePaymentSourceStepInput) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-1.5">
        <Label>{t('revenue.wizard.selectPaymentSource')}</Label>
        {isLoading ? (
        ...
        {errors.paymentSourceId && (
          <p className="text-xs text-[hsl(var(--destructive))]">
            {t(errors.paymentSourceId.message ?? 'revenue.validation.paymentSourceRequired')}
          </p>
        )}
      </div>
