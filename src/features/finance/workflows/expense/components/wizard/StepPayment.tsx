...
import { expensePaymentStepSchema, type ExpensePaymentStepInput } from '../../validation/expense.schema';
...
export function StepPayment({ data, isFirstStep, onNext, onBack, onCancel }: StepPaymentProps) {
  ...
  const { handleSubmit, setValue, watch, formState: { errors } } = useForm<ExpensePaymentStepInput>({
    resolver: zodResolver(expensePaymentStepSchema),
    defaultValues: { paymentMethod: data.paymentMethod },
  });

  const selected = watch('paymentMethod');

  function onSubmit(values: ExpensePaymentStepInput) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-1.5">
        <Label>{t('expense.wizard.selectPaymentMethod')}</Label>
        ...
        {errors.paymentMethod && (
          <p className="text-xs text-[hsl(var(--destructive))]">
            {t(errors.paymentMethod.message ?? 'expense.validation.paymentMethodRequired')}
          </p>
        )}
      </div>
